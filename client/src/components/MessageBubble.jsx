...
const MessageBubble = ({ message, isMe, onDelete, onReact, onReply, repliedMessage }) => {
  ...
  const renderContent = () => {
    if (message.isDeleted) return <p className="italic opacity-80 select-none">Tin nhắn đã bị thu hồi</p>;
    if (isImage) {
      return (
        <img 
    ...
    if (isAudio) {
      return (
        <div className="flex flex-col gap-1.5 min-w-[200px]">
          ...
          <audio src={message.decryptedContent.replace('[AUDIO]', '')} controls className="h-9 w-full rounded outline-none" />
        </div>
      );
    }
    if (isFile) {
      ...
      return (
         <div className="flex items-center gap-3 bg-black/20 p-2.5 rounded-lg border border-white/10 min-w-[240px]">
            ...
            <a href={fileData} download={fileName} className={`p-2 rounded-full transition-colors shrink-0 flex items-center justify-center ${isMe ? 'hover:bg-indigo-400/30 bg-indigo-500/20' : 'hover:bg-slate-600 bg-slate-700'}`} title="Tải xuống tài liệu">
    ...
    return (
      <p className={`whitespace-pre-wrap leading-relaxed text-[15px] ${message.decryptedContent?.startsWith('[') ? 'text-red-300 text-xs italic opacity-80' : ''}`}>
               ...
               <div className="relative">
                 ...
                 {showReactionsMenu && (
                    <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-slate-800 p-2 rounded-full border border-slate-700 shadow-xl flex gap-1 z-30">
                       {emojis.map(emoji => (
                          <span key={emoji} onClick={(e) => { e.stopPropagation(); onReact(message.id, emoji); setShowReactionsMenu(false); }} className="cursor-pointer hover:scale-125 hover:-translate-y-1 transition-transform text-xl px-1">
                             {emoji}
                          </span>
                       ))}
                    </div>
